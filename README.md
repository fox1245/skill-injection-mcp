...
이 프로젝트는 MIT License로 배포됩니다. 자세한 조건은 [LICENSE](LICENSE) 파일을 확인하세요.

`resolve_skills`와 `codex_prompt_hook`이 반환하는 스킬 검색·검증·바인딩 결과는 참고 정보이며,
작업 실행에 대한 승인이나 성공 보장이 아닙니다. 사용자는 실행 전에 후보 스킬의 내용과 결과의 근거를
직접 검토하고, 자신의 환경에 적합한지 판단해야 합니다.

도구 실행과 그 결과에 대한 책임은 사용자에게 있습니다. 여기에는 파일 및 외부 시스템 변경, 데이터
손실, API 키와 자격증명 관리, 외부 서비스 비용, 법적·정책상 의무 및 결과의 적합성 판단이 포함됩니다.
이 소프트웨어는 정확성, 가용성, 보안성 또는 특정 목적에의 적합성을 보장하지 않습니다. MIT License의
보증 부인과 책임 제한도 함께 적용됩니다.
